...
//                                            txn.setStatus(PaymentOrderStatus.COMPLETED);
...
//
//    @Override
//    public Flux<TransactionDto> getPaginatedDepositsByStatusForAdmin(PaymentOrderStatus status, TransactionType type, Integer page, Integer size, String sortBy) {
//        int pageNumber = (page != null && page >= 1) ? page : 1;
//        int pageSize = (size != null && size > 0 && size <= 100) ? size : 10;
//        long offset = (long) (pageNumber - 1) * pageSize;
//
...
//                .switchIfEmpty(Mono.error(new ResourceNotFoundException("Transaction not found")))
//                .flatMap(txn -> {
//                    txn.setStatus(PaymentOrderStatus.CANCELLED);
//                    return transactionRepository.save(txn);
//                })
//                .map(TransactionMapper::toDto)
...
//                    }
//                    if (txn.getStatus() == PaymentOrderStatus.COMPLETED) {
...
//                                                        savedTxn.setStatus(PaymentOrderStatus.COMPLETED);
//
...
//                        return Mono.error(new RuntimeException("Transaction is not a withdrawal"));
//                    }
//                    if (txn.getStatus() == PaymentOrderStatus.COMPLETED || txn.getStatus() == PaymentOrderStatus.CANCELLED) {
//                        return Mono.error(new RuntimeException("Withdrawal already processed"));
//                    }
//
//                    txn.setStatus(PaymentOrderStatus.REJECTED);
...
//
//    @Override
//    public Mono<TransactionDto> changeTransactionStatus(String txnRef, PaymentOrderStatus status, Long approverTelegramId) {
...
//                        }
//
//                    } else if (status == PaymentOrderStatus.REJECTED && txn.getTxnType() == TransactionType.WITHDRAWAL) {
//                        return rejectWithdrawalByAdmin(txnRef, "Rejected by admin", approverTelegramId);
//                    } else if (status == PaymentOrderStatus.CANCELLED && txn.getTxnType() == TransactionType.DEPOSIT) {
